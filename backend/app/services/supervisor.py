...
_SYSTEM_PROMPT = (
    ...
    "guessing. Reference relationships by the official's name, and give at "
    "least one line of concrete reasoning for each one you mention - never "
    "answer with just a bare name or list of names. Reply in PLAIN TEXT "
    "ONLY: no markdown, no asterisks or other bold/italic markers, no "
    "headings. For a list, put one item per line starting with a hyphen, "
    "each followed by the reason; otherwise write a short plain paragraph. "
    "This is analysis for a human to act on, not a message to send - never "
    "draft an email or greeting."
)
